...
export default function DomeGallery({
  ...
  const startAutoRotate = useCallback(() => {
    ...
    const rotate = (currentTime: number) => {
      ...
      const deltaRotation = autoRotateSpeed * deltaTime;
      rotationRef.current.y = (rotationRef.current.y + deltaRotation) % 360;
  ...
  const startInertia = useCallback(
    (vx: number, vy: number) => {
      ...
      const step = () => {
        ...
        if (Math.abs(vX) < stopThreshold && Math.abs(vY) < stopThreshold) {
          ...
          if (autoRotate && !focusedElRef.current) {
    ...
    [dragDampening, maxVerticalRotationDeg, stopInertia, autoRotate, startAutoRotate]
  );
  ...
  }, [enlargeTransitionMs, openedImageBorderRadius, grayscale, autoRotate, startAutoRotate]);
...
.enlarge:hover .contact-info-overlay {
  opacity: 1 !important;
}
    ...
    <>
      ...
      >
        ...
        >
                ...
                >
                  ...
                  >
                    <img
                      ...
                      draggable={false}
                      alt={it.alt}
                      className="w-full h-full object-cover pointer-events-none cursor-target gallery-image"
                      style={{
                        backfaceVisibility: 'hidden',
          ...
          <TargetCursor targetSelector="img.gallery-image" spinDuration={2} hideDefaultCursor={true} />
        </main>
      </div>
    </>
  );
